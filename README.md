...
- 코드 심볼(`{C}`, `{Am}`, `{G/B}` 등)은 MusicXML의 `<harmony>`로 출력합니다.
...
중괄호 안에 코드명을 쓰면 현재 슬롯에 코드 심볼을 붙입니다.
...
- `{C}`는 현재 위치에 C 코드를 표시합니다.
- `{G/B}`는 G/B 슬래시 코드를 표시합니다.
- 코드 심볼은 시간 커서를 움직이지 않습니다.
- MusicXML에서는 `<harmony>`로 출력되므로, MuseScore 같은 악보 프로그램에서 보통 오선 위 코드명으로 표시됩니다.
